// icon-field-colorize — a field of grayscale icons staggers in (staggered fade-in + micro pop), holds a beat, then
// a blue wave sweeps across the whole field extremely fast, followed by orange/green/red waves sweeping downward in turn, ending in four-color bands.
// Matched to bear-app.mp4 0–3s (verified frame by frame: not a hard flip on one frame, but very fast multi-wave color flips).
...
// 16 simple line icons (viewBox 0 0 24 24, solid fill), aiming for recognizable, not refined
...
// Four color waves: blue covers the whole field, then orange/green/red each cover progressively lower row bands → final four-color horizontal bands
const WAVES = [
...
export const IconFieldColorize: React.FC = () => {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      // —— staggered reveal: 10 batches, with jitter within each batch
      ...
      if (ap <= 0) continue;
      // micro pop: 0.55 → 1.07 → 1
      const easeOut = 1 - Math.pow(1 - ap, 3);
      const appearScale = ap < 1 ? 0.55 + easeOut * 0.52 - Math.max(0, ap - 0.72) * 0.25 : 1;

      // —— color waves: each wave sweeps quickly downward from its start row (1.4f per row + slight column lean + jitter)
      const gray = GRAYS[Math.floor(r2 * GRAYS.length)];
      let color = gray;
      let flipPhase = 0; // progress of the latest color flip, used for the pop
