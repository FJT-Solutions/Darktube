// skeleton-reveal — slack-promo 4–15s combined
// three-stage reveal: a full-screen hand-drawn rough-brush doodle placeholder (round-tipped blob line
// art, boiling jitter) → replaced within one beat by a gray-bar skeleton UI window → the skeleton
// message list rolls in, and as the camera pushes in the gray bars "develop" line by line into
// avatar + text content (the last word of the last line arrives half a beat late).
...
// ——— hand-drawn doodle helpers: jitter polylines/circles with a seed and stroke them thick with round caps ———
const wobbleLine = (
...
const wobbleBlobRect = (
  ...
  // round-tipped blob rect: edges jitter mid-segment, corners are large arcs
  const rnd = mulberry32(seed);
...
const Doodle: React.FC<{ boil: number }> = ({ boil }) => {
  const S = boil * 977; // each boil swaps in a fresh set of jitter seeds
  const stroke = {
    fill: 'none' as const,
    stroke: INK,
  ...
  return (
    <svg width={1920} height={1080} style={{ position: 'absolute', inset: 0 }}>
      ...
      {/* main-area message rows: round avatar blobs + wavy text lines */}
...
export const SkeletonReveal: React.FC = () => {
  ...
  const SWAP = 32; // the beat where doodle → skeleton

  // doodle: boiling jitter + pulled back and replaced within one beat (quick shrink-out + fade, accelerating the exit)
  ...
  // word-by-word entrance; the last word of the last line arrives half a beat late (+14 frames)
  ...
  return (
    <AbsoluteFill style={{ background: PAPER, fontFamily: 'Helvetica, Arial, sans-serif', overflow: 'hidden' }}>
      {/* third/second beat: skeleton UI window */}
      {f >= SWAP && (
        <AbsoluteFill
          style={{
      ...
      {/* first beat: hand-drawn doodle placeholder (above the skeleton, shrinks away within one beat to make room) */}
      {doodleVisible && (
        <AbsoluteFill
          style={{
            background: PAPER,
