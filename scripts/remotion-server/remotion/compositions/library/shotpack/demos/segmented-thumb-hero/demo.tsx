// segmented-thumb-hero — close-up on the segmented control's thumb travel as the narrative lead
// Source: perplexity-promo 2.3–5s. The capsule segmented control (Ask/Computer) floats in with a shadow,
// an oversized outlined arrow cursor slides in and clicks, the white thumb slides left→right in ~8f ease-out,
// and the moment it lands, a small icon pops out before the new option while the old icon retracts.
...
// Oversized outlined arrow cursor
...
export const SegmentedThumbHero: React.FC = () => {
  ...
  // ---- Timeline ----
  const FLOAT_IN = 0; // control floats in 0–18
  const CURSOR_IN = 20; // cursor slides in 20–44
  const CLICK = 48; // press
  const SLIDE = 52; // thumb starts sliding 52–60 (~8f)
  const SLIDE_END = 60;

  // Control floats in: springs up from below with a shadow
  ...
  // Control geometry (close-up size)
  const CW = 1080; // control width
  const CH = 220;
  const PAD = 16;
  const SEGW = (CW - PAD * 2) / 2;

  // Cursor slides in: ease-out from off-screen bottom-right onto the Computer segment
  ...
  // On landing: Computer icon pops out (overshoot), Ask icon retracts
