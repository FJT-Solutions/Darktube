// cursor-flyover — Cursor Flyover four-corner tour pointing (motion-lab final cut ported to native Remotion)
// The product screenshot is laid out flat; the camera first fades in on a full overview, then flies to each
// of the four corners for a zoom-in close-up; a shadowed SVG cursor follows to each area to point and
// leaves a click ripple. Each step's transition + hold are equal length, easeInOutCubic.
// Design coordinates 480×270 (DesignStage scales proportionally); the parameter values are calibrated in this coordinate system.
import React from 'react';
import { DesignStage, E, lerp, rand, seg, useT } from '../../_fixtures/Motion';

export const CURSOR_FLYOVER_DURATION = 180; // 6000ms @30fps

// Keyframe accumulator: starts at base, and within each segment [at0,at1] advances toward to (shared file-level helper for the effect)
...
// Camera steps: overview + four corners; cursor targets (world %)
const CAM = [
...
// Top-right line-chart path (same seed as effect.js)
const LINE_D = (() => {
  let dd = '';
...
// Quadrant container (relative positioning within the screenshot)
...
export const CursorFlyover: React.FC = () => {
  ...
  // Click ripple each time the cursor lands
  let click = 0;
  let hold = 0;
  ...
  return (
    <DesignStage bg="#080910" raster="zoom">
      <div style={{ position: 'absolute', inset: 0, background: '#080910', overflow: 'hidden' }}>
        {/* world: carrier of the camera translate+scale, fade-in handled by opacity */}
        <div
          style={{
            position: 'absolute',
            inset: 0,
        ...
        >
          {/* ---- Screenshot placeholder: window chrome + sidebar + quadrant widgets ---- */}
          <div
            style={{
              position: 'absolute',
              left: '4%',
          ...
          >
            {/* Top bar: red/yellow/green lights + URL pill (original capture page content-box: border-bottom extends outside the 9% height) */}
            ...
            {/* Sidebar nav (content-box: border-right extends outside the 15% width) */}
              ...
              {['Overview', 'Traffic', 'Revenue', 'Cohorts', 'Alerts'].map((s, i) => (
                <div
                  ...
                  style={{
                    ...
                    boxSizing: 'content-box', // original capture page is content-box: paddingLeft is added beyond 84%, making the active item's background wider
          ...
          {/* ---- Click ripple (anchored in world, inverse-scaled to keep screen size; content-box: outer diameter 26+2×2=30px matches the original capture page) ---- */}
          <div
            style={{
          ...
          />
          {/* ---- Cursor (anchored in world, inverse-scaled per frame to keep screen size) ---- */}
          <svg
            viewBox="0 0 24 24"
            style={{
