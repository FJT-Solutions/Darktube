// graze-face-tour v2 — the source clip clickup-30.mp4 around 28.5–33s:
// the camera grazes the UI surface at a steep angle in close-up, three segments in relay: sidebar tree → top tab bar → list rows.
// v2 (user feedback): page text initially floats above the interface (lifted in 3D); while airborne it casts a blurred
// same-shape soft shadow on the UI surface; as the camera advances the text settles back onto the interface one by one, the shadow shrinking and fading with height.
...
/* ---------- Float + same-shape soft shadow ----------
 * h = float height (px). The text lifts as a whole up (screen top-left),
 * leaving a blurred, darkened same-shape shadow at its original position; when h→0 they coincide and the shadow disappears. */
...
/* Per-row float height: land = the in-segment moment (0..1) when the row finishes settling; before that it falls from height H with acceleration */
...
/* ---------- Grayscale hand-drawn UI assets ---------- */
...
/* Scene A: sidebar SPACES tree + Recent cards on the right (matching screenshots 1/2/4)
 * v2: tree rows/cards settle onto the interface one by one from the air, in the direction the camera travels (top to bottom) */
const SceneTree: React.FC<{ t?: number }> = ({ t = 1 }) => {
  // Tree row landing times: the camera slides from the top of the tree toward the bottom, so higher row index = later landing
...
/* Scene B: top tab bar + left sidebar navigation (matching screenshots 6/7)
 * v2: tab bar, logo, Home row, and sidebar items settle onto the interface one by one from the air */
...
/* Scene C: list rows (matching the TODAY/TASK NAME area at the bottom right of screenshot 4)
 * v2: the Todo header/TODAY badge/task rows settle in sequence from top to bottom */
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', transform: `translate3d(${x}px, ${y}px, 0)` }}>
          <div style={{ position: 'relative', transform: 'translate(-50%, -50%)' }}>
            {/* Neon glow bleed along the screen edge */}
            {edge === 'left' ? (
              <>
                <div style={{
            ...
            {children}
            {/* Darken the far end: distance fades out in the grazing perspective */}
            <div style={{
              position: 'absolute', inset: 0,
...
/* Background neon-outlined rectangles (dark-scene props) */
  ...
  return (
    <AbsoluteFill style={{ background: '#060608' }}>
      {SEGS.map((s, i) => {
        ...
        // Adjacent segments crossfade: the next fades in over [-FADE, 0] before the boundary, the previous fades out over [SEG_LEN-FADE, SEG_LEN]; no pure black on boundary frames
...
export const GrazeFaceTour: React.FC = () => {
  const frame = useCurrentFrame();
  // Breathing focus band (shallow depth of field): the focus ellipse drifts slightly per segment
  ...
  return (
    <AbsoluteFill style={{ background: '#060608' }}>
      <Stage />
      {/* Screen-space shallow depth of field: everything outside the focus band is blurred */}
      <AbsoluteFill style={{
