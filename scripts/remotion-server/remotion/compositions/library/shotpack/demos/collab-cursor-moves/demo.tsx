// cursor-cast-ensemble — figma 0:05 (cursor-badge-cast) + miro full-film cursor-ensemble-ambience
// Five named colored cursors fly in from off-screen (springy deceleration + name-badge fade-in); once settled
// they keep drifting, pointing, and gathering on the grayscale canvas; grayscale sticky notes serve as ambient
// props; one of them (Rita) live-types a line of text onto a note
// (the typing beat from pitch collaborator-cameo folded in).
...
export const CursorCastEnsemble: React.FC = () => {
  ...
  // Fixed random rotation for the sticky notes
  const noteRots = Array.from({ length: 4 }).map(() => (rnd() - 0.5) * 8);

  // Gather moment: after frame 90 all cursors gather toward the central note
  const gatherT = easeInOut(clamp01((f - 92) / 26));
  const GATHER: [number, number][] = [
    [770, 430], [1160, 420], [800, 640], [1130, 650], [960, 545],
  ];

  // Typing progress (starts after Rita settles, frames 60–130)
  ...
  return (
    <div style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      {/* Infinite dot-grid canvas backdrop */}
      <div style={{
        position: 'absolute', inset: 0,
        backgroundImage: `radial-gradient(${G.line} 2.4px, transparent 2.4px)`,
        backgroundSize: '46px 46px',
      }} />
      {/* Grayscale sticky notes as ambient props */}
      {[
      ...
      {/* Central big note: Rita's typing stage */}
      <div style={{
      ...
      {ACTORS.map((a, i) => {
        ...
        // After settling, keep drifting + occasional "pointing" bursts
