// word-relay-filmstrip v3 — batch 12 single-point tweak: the right large word block (Computer + verb)
// is vertically centered against the current left-column page card's midpoint (y=540) (top 462→402).
// Everything else follows v2: rebuilt against the user screenshots (perplexity-promo01, 7 images):
// ① left-column page cards alternate dark/light and share one height (940x530, spacing 105);
// ② the left column stays still normally, scrolling one step only inside the right-side word-switch window (scroll synced to the switch);
// ③ sizes/font sizes/positions measured from the screenshots: card x=106 width 940, words right-aligned to x≈1710,
//    Didot-style serif 116px, "Computer" fixed on the first line, the verb on the second line grays out and fades in place before swapping.
...
// —— Page card set: alternating dark/light (parity-forced), varied content ——
...
// Fixed dark/light alternating order (odd/even)
...
// Word-switch windows: first word enters f14–30; switches at f62–78 and f108–124
...
export const WordRelayFilmstrip: React.FC = () => {
  ...
  // —— Scroll stepping: still normally, scrolls one step only inside a switch window (ease-in-out) ——
  let stepF = 0;
  SWITCHES.forEach((s) => {
  ...
  // Card layout: center card top y=275, fanning out to both sides; dark/light alternating
  ...
  // —— Right-side word relay: in-place cross-fade (old word grays out and fades, new word fades in, no movement) ——
  ...
  // Each word's life cycle: entrance fade-in (black) → resident black → grays out 14 frames before the next switch point
  // (screenshot 4/6 captured the state "old word already gray, new word not yet visible") → fades out starting at the switch point.
  const smooth = (x: number) => x * x * (3 - 2 * x);
  const wordNodes = WORDS.map((w, i) => {
    const sIn = SWITCHES[i];
    const sOut = i + 1 < SWITCHES.length ? SWITCHES[i + 1] : null;
    // New word fades in over the second half of the window (avoid ghosting over the old word)
    ...
    // Graying: starts 14 frames before the switch point, fully gray at the switch
    const grey = sOut ? interpolate(frame, [sOut - 14, sOut - 2], [0, 1], {
      extrapolateLeft: 'clamp', extrapolateRight: 'clamp',
    }) : 0;
    // Fade-out: completes within the first half of the window after the switch point
  ...
  return (
    <AbsoluteFill style={{ background: '#faf8f3' }}>
      ...
      {/* Right word group: Computer fixed on the first line, the verb swaps in place on the second.
          v3: block total height ≈137 (Computer line) + 140 (word-line container) = 277, its vertical center must align
          with the current page card's midpoint y=540 (card top 275 + card height 530/2) → top = 540 - 277/2 ≈ 402 */}
