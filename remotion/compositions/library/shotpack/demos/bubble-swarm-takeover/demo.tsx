...
// Cutless transition: a swarm of pearlescent bubbles drifts in from off-screen,
// growing until they cover the whole frame while the page is simultaneously
// "whitewashed"; the scene switch hides at the peak of coverage, and after the
// bubbles disperse the new scene is already there. Includes i18n text-capsule variants.
...
const PEAK = 75; // peak coverage frame (hidden cut point)
const W = 1920;
const H = 1080;
const CX = W / 2;
const CY = H / 2;

// Pearlescent hue variations (pink/purple/blue/cyan)
const TINTS = [
...
const makeBubbles = (): BubbleSpec[] => {
  ...
  // 34 regular bubbles (three depth layers)
  for (let i = 0; i < 34; i++) {
  ...
  // 6 giant bubbles that cover the screen at peak (grid targets guarantee coverage)
  const grid = [
...
const Bubble: React.FC<{ spec: BubbleSpec; frame: number }> = ({ spec, frame }) => {
  ...
  // Disperses outward after the peak (accelerating exit)
...
export const BubbleSwarmTakeover: React.FC = () => {
  ...
  // Page "whitewash": overall brightness lifts as it approaches the peak
  ...
  return (
    <AbsoluteFill style={{ background: '#ececea', overflow: 'hidden' }}>
      ...
      {/* Whitewash layer sits above the page, below the bubbles */}
      <AbsoluteFill style={{ background: '#ffffff', opacity: whiteout }} />
      {/* Far/mid-layer bubbles */}
      {BUBBLES.filter((b) => b.z < 2).map((b, i) => (
        <Bubble key={i} spec={b} frame={frame} />
      ))}
      {/* i18n text capsules drifting among the mid-layer bubbles */}
      {CAPSULES.map((c, i) => {
      ...
      {/* Foreground giant bubbles (out of focus) */}
      {BUBBLES.filter((b) => b.z === 2).map((b, i) => (
        <Bubble key={i} spec={b} frame={frame} />
      ))}
    </AbsoluteFill>
