// product-card-progressive-assemble — Progressive Assemble fields landing one by one (motion-lab final cut ported to native Remotion)
// the detail card builds itself as if each field is grabbed into place: image → title → breadcrumb pills
// popping in one by one → the price appears then gets demoted with a strikethrough,
// while the accent-colored new price springs out → body text reveals line by line + accent highlight
// blocks sweep left to right → swatches light up.
// The whole card slowly pushes forward in scale to keep breathing. Design coordinates 480×270 (DesignStage scales proportionally).
...
const F = (f: number) => f / 60; // recipe frames → t

// field landing style: rise = moves up 6px + fade in; pop = outBack bounces from 0.4 to 1
...
// description lines: [text, whether marker-highlighted]
...
export const ProductCardProgressiveAssemble: React.FC = () => {
  const t = useT();
  // whole-card breathing push
  const push = seg(t, 0, 0.75, E.outQuad);
  // price demotion (f=26): old price strikethrough, shrinks and turns gray; new price springs out
  const cut = t >= F(26);
