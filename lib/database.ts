...
export async function ensureDarkClipsTablesExist() {
    await pool.query(`
        ...
        ALTER TABLE public.dark_clips_presets ADD COLUMN IF NOT EXISTS arrows_list JSONB DEFAULT '[]'::jsonb;
    ...
    if (preset.id) {
        const query = `
            UPDATE public.dark_clips_presets SET
                ...
                arrows_list = COALESCE($9, arrows_list),
        ...
        const values = [
            ...
            preset.arrows_list ? JSON.stringify(preset.arrows_list) : null,
            preset.is_default,
            preset.id
        ]
        const { rows } = await pool.query(query, values)
        ...
        const query = `
            INSERT INTO public.dark_clips_presets (
                ...
                video_placement, background_style, watermark_style, footer_style, arrows_style, arrows_list, is_default
        ...
        const values = [
            ...
            JSON.stringify(preset.arrows_list || []),
...
export async function getDarkClipPresets(userId?: string): Promise<DarkClipPreset[]> {
    ...
    return rows.map((r: any) => ({
        ...
        arrows_list: typeof r.arrows_list === 'string' ? JSON.parse(r.arrows_list) : (r.arrows_list || [])
    }))
}
