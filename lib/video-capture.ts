...
export const VideoCaptureService = {
     ...
     * @param sessionCookies - Optional array of cookies from the user's browser (from Chrome extension).
     *                         Format: [{name, value, domain, path, secure, expirationDate}]
     */
    async downloadFromUrl(url: string, sessionCookies?: Record<string, string>[]): Promise<DownloadResult> {
        ...
        try {
            ...
            // Write session cookies from the browser extension (Netscape format for yt-dlp)
            let sessionCookiePath: string | undefined;
            let sessionCookieCleanup = () => {};
            if (sessionCookies && sessionCookies.length > 0) {
                try {
                    const baseTmp2 = process.env.NODE_ENV === 'production' ? '/app/tmp' : path.join(process.cwd(), 'tmp');
                    sessionCookiePath = path.join(baseTmp2, `session_cookies_${Date.now()}.txt`);
                    const lines = [
                        '# Netscape HTTP Cookie File',
                        '# Generated by DarkTube Extension',
                        '',
                        ...sessionCookies.map((c) => {
                            ...
                            const expiry = c.expirationDate ? Math.floor(Number(c.expirationDate)) : 9999999999;
                            return `${domain}\t${includeSubdomains}\t${cookiePath}\t${secure}\t${expiry}\t${c.name}\t${c.value}`;
                        })
                    ];
                    fs.writeFileSync(sessionCookiePath, lines.join('\n'));
                    console.log(`[VideoCaptureService] Usando ${sessionCookies.length} cookies da sessão do usuário (extensão)`);
                    sessionCookieCleanup = () => {
                        try { if (sessionCookiePath && fs.existsSync(sessionCookiePath)) fs.unlinkSync(sessionCookiePath); } catch {}
                    };
                } catch (ckErr) {
                    console.warn('[VideoCaptureService] Falha ao escrever cookies de sessão:', ckErr);
                }
            }

            const effectiveCookiePath = sessionCookiePath || cookies;
            ...
            try {
                ...
                await execFilePromise(
                    getYtDlpPath(),
                    [
                        ...getUniversalYtDlpArgs(url, effectiveCookiePath),
                        '-f', 'best[ext=mp4]/best', 
                        '-o', videoPath, 
                        url
                    ],
